...
type Tab = "appearance" | "integrations" | "session";
...
export function SettingsPanel({ isOpen, onClose }: SettingsPanelProps) {
  ...
  return (
    ...
    >
      <div className="w-[480px] max-h-[80vh] bg-bg-overlay border border-border-default rounded-lg shadow-2xl flex flex-col overflow-hidden">
        {/* Header */}
        ...
        <div className="flex border-b border-border-default shrink-0">
          {(["appearance", "integrations", "session"] as Tab[]).map((t) => (
            <button
              key={t}
              onClick={() => setTab(t)}
              className={cn(
                "px-4 py-2 text-xs capitalize transition-colors border-b-2 -mb-px",
                tab === t
