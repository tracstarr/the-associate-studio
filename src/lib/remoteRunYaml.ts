      ...
      - name: Fetch issue and extract prompt
        ...
        run: |
          ...
          if [ "$ISSUE_TYPE" = "github" ]; then
            ...
            python3 -c "import json,os;issue=os.environ['ISSUE_NUMBER'];q={'query':'{issues(filter:{identifier:{eq:\\"'+issue+'\\"}}){nodes{title description}}}' };f=open('/tmp/linear_query.json','w');json.dump(q,f);f.close()"
            RESPONSE=$(curl -fsSL \\
              -H "Authorization: $LINEAR_API_KEY" \\
              -H "Content-Type: application/json" \\
              -d @/tmp/linear_query.json \\
              "https://api.linear.app/graphql")
            echo "$RESPONSE" | python3 -c "import sys,json;d=json.load(sys.stdin);node=d['data']['issues']['nodes'][0];print(node['title'])" > /tmp/issue_title.txt
            echo "$RESPONSE" | python3 -c "import sys,json;d=json.load(sys.stdin);node=d['data']['issues']['nodes'][0];print(node.get('description') or '')" > /tmp/issue_body.txt
          fi
